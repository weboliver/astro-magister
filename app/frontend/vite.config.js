...
const allowedHosts = (process.env.VITE_ALLOWED_HOSTS || 'localhost,127.0.0.1,raspissd,raspissd.local')
  .split(',')
  .map((host) => host.trim())
  .filter(Boolean)
...
export default defineConfig({
  ...
  server: {
    ...
    port: 5173,
    proxy: {
      // Only proxy API requests to the FastAPI backend during development.
