...
test('poweruser can view houses chart image', async ({ page }) => {
  test.setTimeout(90000);

  await page.goto('/');
  await expect(page.getByRole('heading', { name: 'Startseite' })).toBeVisible({ timeout: 10000 });

  await page.locator('span.app-nav-link').filter({ hasText: 'Häuser' }).click();
  await expect(page.getByRole('heading', { name: 'Häuser', exact: true })).toBeVisible({ timeout: 5000 });
  ...
  const chartImg = page.locator('img[alt="Häuser Diagramm"]');
  try {
