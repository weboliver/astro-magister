...
test('poweruser can view age points chart image', async ({ page }) => {
  test.setTimeout(90000);

  await page.goto('/');
  await expect(page.getByRole('heading', { name: 'Startseite' })).toBeVisible({ timeout: 10000 });

  await page.locator('span.app-nav-link').filter({ hasText: 'Alterspunkte' }).click();
  await expect(page.getByRole('heading', { name: 'Age Points', exact: true })).toBeVisible({ timeout: 5000 });
  ...
  const chartImg = page.locator('img[alt="Age Points Diagramm"]');
  try {
