...
except ImportError:  # pragma: no cover - best effort translation helper
...
class OpPanel(Gtk.VBox):
    ...
    def append_mult_group(self,name,pos,page,offset=3):
        ...
        button.catalog_page = page
        button.catalog_name = name
    ...
    def on_catalog_button_toggled(self, button):
        pmode = ['','master','click']
        page = getattr(button, 'catalog_page', 0)
        name = getattr(button, 'catalog_name', '')
        self.current_button = button
        ...
        page_widget = self.notebook.get_nth_page(page)
        
        # Try to get selection from page directly (if it's a TreeView)
        try:
            sel = page_widget.get_selection()
            m,i = sel.get_selected()
            if i is None:
                sel.select_path(0)
            sel.emit('changed')
            curr.set_list(button.catalog_name)
        except AttributeError:
            # Page is a container with multiple TreeViews
            if curr.opmode == 'double':
                left,_,right = page_widget.get_children()
                ...
                if curr.clickmode == 'click':
                    ...
                    if i is None or m.get_path(i).get_indices() != (2,): #nodal
                        right.get_selection().select_path((1,))
                right.get_selection().emit('changed')
                curr.set_list(button.catalog_name)
            elif curr.opmode == 'triple':
                left,_,up,_,right = page_widget.get_children()
                if curr.clickmode == 'click':
                    left.get_selection().select_path((0,))
                    right.get_selection().select_path((0,))
                else:
