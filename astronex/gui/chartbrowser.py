...
class ChartBrowserWindow(Gtk.Window):
    def __init__(self,parent):
        ...
        self.notebook.connect('switch-page',self.page_select)

        label = Gtk.Label(_("Explorador"))
        label.set_angle(90)
        self.notebook.append_page(BrowserPanel(parent),label)
        
        label = Gtk.Label(_("Mezclador"))
        label.set_angle(90)
        self.notebook.append_page(MixerPanel(parent),label)

        label = Gtk.Label(_("Importacion AAF"))
        label.set_angle(90)
        self.notebook.append_page(ImportPanel(parent),label)

        label = Gtk.Label(_("Parejas"))
        label.set_angle(90)
        self.notebook.append_page(CouplesPanel(parent),label)
    ...
    def cb_exit(self,e,parent):
        parent.browser = None
        page1 = self.notebook.get_nth_page(1)
        if page1 and hasattr(page1, 'changes') and page1.changes:
            boss.mpanel.browser.tables.emit('changed')
            boss.mpanel.browser.relist('')
        page3 = self.notebook.get_nth_page(3)
        if page3 and hasattr(page3, 'save_couples'):
            page3.save_couples()
        return False
...
class  BrowserPanel(Gtk.HBox): 
    def __init__(self,parent):
        ...
        self.tables = create_combo_with_entry(liststore, editable=False)
...
class ChartSnapshot(Gtk.DrawingArea):
    ...
    def redraw(self): 
        w = self.allocation.width
        h = self.allocation.height
        self.window.invalidate_rect(Gdk.Rectangle(0,0,w,h),False)
