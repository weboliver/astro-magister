...
from .gtk_helpers import apply_bg_color, get_combo_active_text
curr = None
boss = None
...
class Slot(Gtk.VBox):
    ...
    def __init__(self,id):
        ...
        img.set_from_file(str(imgfile))
        ev.add(img)
        apply_bg_color(ev, "white", "slot-eye-bg")
        ...
        eb = Gtk.EventBox()
        eb.add(table)
        apply_bg_color(eb, "white", "slot-main-bg")
    ...
    def on_eye_clicked(self,eye,event):
        if event.type == Gdk.BUTTON_PRESS and event.button == 1:
            # Only list MRU charts that still exist in the database
            names = []
            for ch in curr.pool:
                try:
                    if curr.datab.chart_exists_any_table(ch.first, ch.last):
                        names.append(" ".join([ch.first, ch.last]))
                except Exception:
                    # If DB check fails, skip the entry to avoid stale items
                    pass
            menu = Gtk.Menu()
            ...
            for buf in names:
                ...
                menu_items.show()
            # GTK3: use popup_at_pointer to avoid deprecated popup signature issues
    ...
    def on_eye_menu_activate(self,menuitem,menu):
        active = Slot.storage
        ix = 0
        # Gtk3 menu item: use its label rather than child attribute
    ...
    def on_storage_clicked(self,but):
        ...
        apply_bg_color(self.storage_but, "white", "slot-storage-bg")
        apply_bg_color(other.storage_but, "#f6f7fe", "slot-storage-bg")
    ...
    def on_slot_clicked(self,slot,event):
        ...
        if event.type == Gdk.BUTTON_PRESS and event.button == 3:
            # Use popup_at_pointer for GTK3.22+ compatibility
            self.menu.popup_at_pointer(event)
            return True
        if event.type == Gdk.BUTTON_PRESS and event.button == 1:
            if self.wname != MainPanel.active_slot:
                other = MainPanel.pool[self.other]
                apply_bg_color(self.eb, "white", "slot-main-bg")
                apply_bg_color(other.eb, "#f6f7fe", "slot-main-bg")
                self.eye.show()
                other.eye.hide()
...
class ChartBrowser(Gtk.VBox):
    def __init__(self,ap_path,font):
        ...
        chartlist = curr.datab.get_chartlist(get_combo_active_text(self.tables))
    ...
    def relist(self,new):
        ...
        for c in tablelist:
            liststore.append([c])
        if not new:
            new = get_combo_active_text(self.tables)
        index = 0
        for i,r in enumerate(liststore):
            if r[0] == new:
                index = i
    ...
    def on_view_clicked(self,view,event):
        if event.type == Gdk.BUTTON_PRESS and event.button == 2:
            ...
            try:
                ...
                table = get_combo_active_text(self.tables)
            chart = curr.newchart()
    ...
    def on_menuitem_activate(self,menuitem,item):
        ...
        if item == _('Copiar') or item == _('Cortar'):
            ...
            if item == _('Cortar'):
                ...
                # Reload the chart list to reflect the deletion
                self.on_tables_changed(self.tables)
        else: # paste
    ...
    def on_chart_activated(self,view,path,col):
        model,iter = view.get_selection().get_selected()
        id = model.get_value(iter,1)
        # Check if model has 3 columns (has table column)
...
class MainPanel(Gtk.VBox):
    ...
    def update_slot_label(slot,chart):
        ...
        slot.reglbl.set_text(chart.country+' ('+region+')')
    ...
    def actualize_pool(slot,chart):
        ...
        if curr.curr_op == "sec_prog" and boss.da and boss.da.panel:
            boss.da.panel.nowbut.emit('clicked')
