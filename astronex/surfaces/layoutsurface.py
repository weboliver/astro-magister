...
from .. countries import cata_reg, t
...
class DrawMaster(Gtk.Layout):
    ...
    def __init__(self,boss):
        ...
        self.panel = None  # Will be created as overlay window when needed
        self.panel_window = None  # The Gtk.Window containing the panel
    ...
    def on_da_clicked(self,da,event):
        ...
        if event.type == Gdk._2BUTTON_PRESS and event.button == 1:
            ...
            return True
        elif event.type == Gdk.BUTTON_PRESS and event.button == 1:
            ...
            if showAP is None:
                return True
            w = da.get_allocated_width() / 2
            h = da.get_allocated_height() / 2
            if curr.opmode == 'simple' and curr.curr_op in peops:
                ...
                return False
            return True
        elif event.type == Gdk.BUTTON_PRESS and event.button == 2:
            ...
            if curr.curr_op in bios or curr.opmode != 'simple':
                return False
            # GTK3: get_pointer() returns (window, x, y, state)
    ...
    def redraw(self):
        ...
        #    x = int(self.ha.lower)
        #    y = int(self.va.lower)
        #    self.bin_window.invalidate_rect(Gdk.Rectangle(x,y,w,h),False)
        #else:
        #    w = self.allocation.width
        #    h = self.allocation.height
        #    self.bin_window.invalidate_rect(Gdk.Rectangle(0,0,w,h),False)
        if self.diadavisible and self.diada.get_child():
            self.diada.get_child().queue_draw()
        if boss.da.hselvisible:
            if boss.da.hsel.get_child():
    ...
    def show_panel(self,menuitem=None):
        ...
        # Create a separate window for the calendar panel - NO DECORATION
        self.panel_window = Gtk.Window(Gtk.WindowType.TOPLEVEL)
        self.panel_window.set_type_hint(Gdk.WindowTypeHint.DIALOG)
        self.panel_window.set_decorated(False)  # No window decorations
        self.panel_window.set_transient_for(boss.mainwin)  # Attached to main window
        self.panel_window.set_skip_taskbar_hint(True)
        self.panel_window.set_skip_pager_hint(True)
        self.panel_window.set_destroy_with_parent(False)  # Don't auto-close
        # Don't use set_keep_above - it can interfere with transient behavior
        
        # Handler to bring panel to front when clicked anywhere
        def keep_panel_above(widget, event):
            if self.panel_window and self.panel_window.get_visible():
                self.panel_window.present()
            return False  # Propagate event
        
        # Register on both main window and drawing area to catch all clicks
        if not hasattr(self, '_panel_click_handlers'):
            self._panel_click_handlers = []
            handler1 = boss.mainwin.connect('button-press-event', keep_panel_above)
            handler2 = self.connect('button-press-event', keep_panel_above)
            self._panel_click_handlers.append((boss.mainwin, handler1))
            self._panel_click_handlers.append((self, handler2))
        
        # Enable all events on the window
        self.panel_window.set_events(
        ...
        self.panel_window.add(self.panel)
        # Set size to fit the calendar + buttons
        self.panel_window.set_size_request(320, 170)
        self.panel_window.set_default_size(320, 170)
        
        # Position: X = DrawMaster.x + navigation_width + 10, Y = DrawMaster.y + 5
        try:
            # Get main window position
            main_x, main_y = boss.mainwin.get_position()
            # Get navigation panel width (mpanel)
            nav_width = boss.mpanel.get_allocation().width
            # Get DrawMaster position within main window
            alloc = self.get_allocation()
            # Calculate absolute position on screen
            # X = main_x + nav_width + 10
            # Y = main_y + alloc.y + 5
            panel_x = main_x + nav_width + 40
            panel_y = main_y + alloc.y + 60
            self.panel_window.move(panel_x, panel_y)
        except Exception as e:
            self.panel_window.move(5, 5)
    ...
    def hide_panel(self,menuitem=None):
        ...
        # Disconnect click handlers if they exist
        if hasattr(self, '_panel_click_handlers'):
            for widget, handler_id in self._panel_click_handlers:
    ...
    def dispatch(self, da, cr):
        """Main drawing dispatch - protected against crashes"""
        try:
            ...
            if DrawMaster.overlay:
                rgba = Gdk.RGBA()
                overlay_hex = '#'+self.opts.overlay
                if not rgba.parse(overlay_hex):
                    rgba.parse('#000000')
                ovcol = [rgba.red, rgba.green, rgba.blue, 0.5]
            ...
            import sys
            print(f"!!! CRASH PREVENTED in dispatch(): {e}", file=sys.stderr, flush=True)
            import traceback
            ...
            try:
                w = self.get_allocated_width()
                h = self.get_allocated_height()
                cr.set_source_rgb(1.0, 0.9, 0.9)  # Light red background
                cr.rectangle(0,0,w,h)
                
                cr.set_source_rgb(0.8, 0, 0)  # Dark red text
                layout = PangoCairo.create_layout(cr)
                font = Pango.FontDescription("Sans")
                font.set_size(12*Pango.SCALE)
                layout.set_font_description(font)
                layout.set_text(f"Drawing error: {str(e)[:100]}")
                cr.move_to(10, 10)
    ...
    def d_now_date(self,cr,w,h):
        """Draw current date label - protected against crashes"""
        try:
            if self.diadavisible:
                return
            ...
            # ENABLED: year_regent is now safe with calc() caching in pysw.py
            # The cache prevents re-entrant calls to the C extension
            if True:
                try:
                    ...
                    import sys
                    print(f"WARNING: year_regent display failed: {e}", file=sys.stderr, flush=True)
                    pass
        except Exception as e:
            import sys
            print(f"ERROR in d_now_date: {e}", file=sys.stderr, flush=True)
            import traceback
            traceback.print_exc(file=sys.stderr)
...
class ChangeDatePanel(Gtk.VBox):
    ...
    def __init__(self,parent):
        ...
        try:
            self.calendar.set_week_start(Gtk.CalendarWeekday.MONDAY)
