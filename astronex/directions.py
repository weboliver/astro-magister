...
def solar_rev(boss):
    ...
    boss.state.calcdt.settz(zone)
    boss.state.calcdt.setdt(dt)
    # Use setprogchart to calculate solar revolution with proper chart setup
    boss.state.setprogchart(boss.state.curr_chart)
    if boss.da and boss.da.panel:
...
def sec_prog(boss):
    ...
    if boss.da and boss.da.panel and not boss.da.sec_alltimes:
        boss.da.panel.set_date_only(birthday)
    elif boss.da and not boss.da.panel:
        pass
    elif not boss.da:
        pass
    else:
