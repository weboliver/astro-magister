...
class Manager(object):
    ...
    def set_mainwin(self,mainwin):
        ...
        # Deferred showing - handle GTK.Layout rendering carefully
        ...
        def show_main_window():
            """Show main window after initialization - avoid calling show_all() which crashes on Gtk.Layout"""
            try:
                # Show the main window
                mainwin.show()
                time.sleep(0.1)
                
                # Show HBox and its children carefully
                if hasattr(mainwin, 'get_children'):
                    for child in mainwin.get_children():
                        child.show()
                        time.sleep(0.1)
                        # Show HBox children: VBox needs show_all(), ScrolledWindow only show()
                        if hasattr(child, 'get_children'):
                            for subchild in child.get_children():
                                subchild_type = type(subchild).__name__
                                if subchild_type == "VBox":
                                    # VBox contains Toolbar and MainPanel - use show_all()
                                    subchild.show_all()
                                    time.sleep(0.15)
                                elif subchild_type == "ScrolledWindow":
                                    # ScrolledWindow - show_all to display DrawMaster (unstable but shows horoscope)
                                    subchild.show_all()
                                    time.sleep(0.1)
                
                # Trigger redraw of the DrawMaster to display the chart
                if hasattr(self, 'da') and hasattr(self.da, 'queue_draw'):
                    self.da.queue_draw()
                    # Queue multiple redraws to ensure rendering with add_with_viewport
                    gobject.timeout_add(100, lambda: self.da.queue_draw() or False)
                    gobject.timeout_add(300, lambda: self.da.queue_draw() or False)
                    gobject.timeout_add(600, lambda: self.da.queue_draw() or False)
            except Exception:
                pass
            return False
        ...
        # Call with delay to ensure widgets are initialized
        gobject.timeout_add(50, safe_show)
        
        # Backup redraw for slow systems - do NOT re-call safe_show()
        def redraw_backup():
            if hasattr(self, 'da') and hasattr(self.da, 'queue_draw'):
                try:
                    self.da.queue_draw()
        ...
        gobject.timeout_add(1000, redraw_backup)
        gobject.timeout_add(2000, redraw_backup)
        gobject.timeout_add(3000, redraw_backup)
