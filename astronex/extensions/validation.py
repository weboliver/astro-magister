...
class MaskEntry(Gtk.Entry):
    ...
    def __init__(self):
        ...
        # internal flags/structures must exist before signal handlers run
    ...
    def set_text(self, text):
        """Programmatic set_text without firing mask handlers (avoids GI int warnings)."""
        self._block_insert = True
        self._block_delete = True
        buf = self.get_buffer()
        if buf is not None:
            # Bypass Editable.insert_text; set via the underlying buffer to keep GI happy
            buf.set_text(text, -1)
        else:
    ...
    def _really_delete_text(self, start, end):
        ...
        buf = self.get_buffer()
        if buf is not None:
            # operate directly on the entry buffer to avoid Gtk.Editable.delete_text overrides
    ...
    def _really_insert_text(self, text, position):
        ...
        buf = self.get_buffer()
        if buf is not None:
            # insert directly into the buffer to avoid Gtk.Editable.insert_text overrides
            buf.insert_text(position, text, -1)
        else:
