...
class PaarWabeMixin(object):
    def __init__(self,zodiac):
        ...
        # Cache für Paarwabe-Berechnungen, Schlüssel: (f_first,f_last,f_date,m_first,m_last,m_date)
        self._pw_cache = {}
    ...
    def _paar_cache_key(self, chartob):
        try:
            f = chartob.chart
            m = chartob.click
            return (getattr(f, 'first', None), getattr(f, 'last', None), getattr(f, 'date', None),
                    getattr(m, 'first', None), getattr(m, 'last', None), getattr(m, 'date', None))
        except Exception:
            return None

    def _get_paarwabe_data(self, chartob):
        key = self._paar_cache_key(chartob)
        if key and key in self._pw_cache:
            return self._pw_cache[key]

        # Teure Berechnungen nur einmal pro Paar durchführen
        ...
        if key:
            self._pw_cache[key] = data
        return data

    def paarwabe_plot(self,cr,width,height,chartob):
        name = self.surface.__class__.__name__
        cr.new_path()  # reset any previous drawing path before starting
        cx,cy = width/2,height/2
        if name == "DrawMaster":
            r = min(cx,cy)*0.80
        else:
            r = min(cx,cy)*0.8 
        data = self._get_paarwabe_data(chartob)
        paar = data['paar']
        planpaar = data['planpaar']
        cells = data['cells']
        sump = data['sump']
        asp = data['asp']
        colglob = data['colglob']
        inasp = data['inasp']
        sumshe = data['sumshe']
        sumhe = data['sumhe']
    ...
    def make_pwhexagon(self,cr,r,x,y,co,ci,il,ir,ll,lr,paar,inasp):
        ...
        cr.translate(x,y)
        cr.new_path()  # ensure no stray segments from previous draws
        ...
        # Ensure path does not leak to subsequent draws
        cr.new_path()
        ...
        cr.new_path()
        cr.restore()
        cr.new_path()  # clear path after drawing to avoid carry-over lines
    ...
    def make_innerpw(self,cr,r,x,y,co,ci,pa,pb,a):
        ...
        cr.set_line_width(r*0.04)
        cr.translate(x,y)
        cr.new_path()  # reset path to avoid inherited segments
        cr.move_to(0,-r)
        cr.line_to(r*math.cos(-30*RAD),r*math.sin(-30*RAD))
        cr.line_to(r*math.cos(30*RAD),r*math.sin(30*RAD))
        cr.line_to(r*math.cos(90*RAD),r*math.sin(90*RAD))
    ...
    def make_yellowpw(self,cr,r,x,y,n,stroke=False,ix=0,fs=16):
            ...
            cr.set_line_width(r*0.04)
            cr.translate(x,y)
            cr.new_path()  # clear any previous segments before drawing the hexagon
    ...
    def crown_plot(self,cr,width,height,asp,col):
        cx,cy = width/2,height/2
        cr.new_path()  # reset path before drawing crown background
        cr.set_line_width(0)
        if cx < cy:
            r = max(cx,cy)*0.95 
        else:
